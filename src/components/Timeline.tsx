  ...
  return <div className="h-screen flex flex-col">
      ...
      <ScrollArea className="flex-1 pr-2">
        <div className="space-y-3 pb-4 px-[12px]">
          {scenes.map((scene, index) => <Card key={scene.id} className={`timeline-scene cursor-pointer transition-all duration-200 group hover:shadow-md ${currentScene === index ? 'ring-2 ring-video-primary shadow-lg bg-video-primary/5' : ''} ${draggedScene === index ? 'dragging' : ''} ${scene.isRendering ? 'opacity-75' : ''}`} onClick={() => onSceneSelect(index)} draggable onDragStart={() => handleDragStart(index)} onDragEnd={handleDragEnd} onDragOver={handleDragOver}>
